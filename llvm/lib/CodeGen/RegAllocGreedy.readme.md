...
逐个驱逐：跳过已被前一个 unit 处理掉的重复项（`hasPhys` 为 false 表示已 unassign）；调用 `Matrix->unassign` 解除分配；断言驱逐合法性（被驱逐者的 cascade 必须 < 当前 Cascade，除非是 urgent eviction 或可溢出性更差）；为被驱逐的虚拟寄存器设置新 cascade，统计计数，并加入 NewVRegs 队列等待重新分配。
...
**功能**: 在给定 `CostPerUseLimit` 下，计算 `AllocationOrder` 中值得进入驱逐分析的候选范围。若该寄存器类不存在 cost 严格小于 limit 的寄存器，则返回 `nullopt`；否则返回裁剪后的 `OrderLimit`。

**核心目的**: 它不只是一个减少遍历次数的性能优化，还负责落实驱逐策略中的成本边界：当当前虚拟寄存器已经有一个可用但较昂贵的物理寄存器时，只有在驱逐能够换来一个**严格更便宜**的物理寄存器时，驱逐才值得尝试。它避免为了同样昂贵或更昂贵的寄存器驱逐其他 live range，造成额外的重新分配、分裂或 spill。

典型调用发生在 `tryAssign`：已经找到一个空闲的 `PhysReg` 后，如果其 `RegCosts[PhysReg]` 非零，就以该 cost 作为 `CostPerUseLimit` 调用 `tryEvict`。因此候选必须满足：

```text
RegCosts[Candidate] < RegCosts[当前空闲 PhysReg]
```

例如 allocation order 的 cost 为 `[0, 0, 1, 1, 1]`，而当前已有 cost=1 的空闲寄存器，则驱逐搜索只需分析前两个 cost=0 的候选。若无法从它们中找到合法驱逐对象，就直接保留已有的 cost=1 寄存器；没有必要驱逐其他 live range 去换另一个 cost=1 的寄存器。
...
`uint8_t(~0u)` 即 255，表示无限制。当有限制时，取 VirtReg 所在寄存器类的最小代价 MinCost；若最小代价已 >= 限制，则该类中没有任何寄存器能满足“cost 严格小于 limit”，返回 `nullopt` 通知 eviction advisor 放弃本次驱逐搜索。这不表示该寄存器类完全无法分配，只表示找不到符合当前成本目标的候选。
...
若 AllocationOrder 末尾的寄存器代价超限（说明尾部存在一组不满足成本目标的寄存器），则把上限设为 `getLastCostChange(RC)`——即寄存器类中最后一次代价变化的分界点，整段跳过同 cost 的昂贵尾部。寄存器类常具有“长尾同代价”特性；提前裁剪可避免对这些必然会被 `canAllocatePhysReg` 拒绝的候选逐个执行干扰查询和驱逐收益计算。

这里不是假设整个 `AllocationOrder` 都严格按 cost 排序。`RegisterClassInfo` 保留目标给出的主要顺序，并通常把 CSR alias 放到后面；`getLastCostChange` 只利用“最后一个变化点之后具有相同 cost”这一事实裁剪尾部。裁剪前范围内若仍有个别超限候选，后续由 `canAllocatePhysReg` 逐个过滤。
...
| `RegCosts` | [PhysReg] | 目标描述提供的 `CostPerUse`，表示使用该寄存器可能引入的额外指令代价，例如更长的指令编码 |
| `std::optional<unsigned>` | - | `nullopt` 表示没有 cost 严格低于当前 limit 的候选；数值表示驱逐搜索的候选范围上限 |

---

### 策略作用与性能作用

1. **保证驱逐有成本收益**：已有空闲寄存器时，只为严格更便宜的候选考虑驱逐，避免无收益地扰动其他 live range。
2. **早退省时**：若整类最小代价都不低于 limit，直接返回 `nullopt`，避免进入后续驱逐分析。
3. **长尾裁剪**：用 `getLastCostChange` 一次跳过同 cost 的昂贵尾部，减少干扰查询和 eviction cost 计算。
4. **无限制时保证成功率**：`CostPerUseLimit==255` 时不施加成本边界，保留全量候选。
5. **与逐项过滤分工**：`getOrderLimit` 做寄存器类级别的粗裁剪，`canAllocatePhysReg` 对范围内候选执行精确 cost 检查，并处理首次启用 CSR 等动态条件。
...
| AllocationOrder 尾部 | 最后一个 cost 变化点之后的寄存器具有相同 cost | 函数只裁剪这个同 cost 尾部，并不要求整个顺序按 cost 全局排序 |
| getLastCostChange 语义 | 返回最后一次 cost 变化的索引 | 它是尾部边界，不是“所有便宜寄存器结束位置”；范围内仍由 `canAllocatePhysReg` 精确过滤 |
...
`uint8_t(~0u)` 用作“无限”哨兵值。当前代码中 `CostPerUseLimit` 的主要来源有三种：

1. `selectOrSplitImpl` 初始化为 255，普通驱逐搜索不限制寄存器 cost。
2. `tryAssign` 已找到一个非零 cost 的空闲寄存器时，以该寄存器的 cost 为 limit，再尝试通过驱逐寻找严格更便宜的替代者。
3. `tryAssignCSRFirstTime` 决定 spill 比首次启用 CSR 更划算时，将 limit 设置为 1，避免后续 `tryEvict` 又通过驱逐选择昂贵寄存器或首次启用 CSR。

需要区分两种成本：`RegCosts` 是目标提供的静态 `CostPerUse`；首次启用 callee-saved register 带来的函数级 prologue/epilogue 成本由 `CSRCost` 及 `isUnusedCalleeSavedReg` 的动态逻辑处理，并不是简单包含在 `RegCosts` 中。
